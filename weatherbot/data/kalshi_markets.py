...
def _build_series_map() -> Dict[str, Tuple[str, str]]:
    """Invert cities.json kalshi_series into a flat series→(city, metric) lookup."""
    from weatherbot.data.weather import CITY_CONFIG
    result: Dict[str, Tuple[str, str]] = {}
    for city_key, city in CITY_CONFIG.items():
        for metric, series_list in city.get("kalshi_series", {}).items():
            for series in series_list:
                result[series] = (city_key, metric)
    return result

KNOWN_SERIES_MAP: Dict[str, Tuple[str, str]] = _build_series_map()
        ...
        try:
            while True:
                ...
                for m in raw_markets:
                    ...
                    if yes_ask_size < settings.MIN_ASK_SIZE:
                        logger.info(
                            f"LIQUIDITY SKIP {ticker}: ask_size={yes_ask_size:.0f} < {settings.MIN_ASK_SIZE}")
                        report.filtered.append(FilteredMarket(
                            ticker=ticker, city=city_key, title=title,
                    ...
                    if volume_24h < settings.MIN_VOLUME_24H:
                        logger.info(
                            f"LIQUIDITY SKIP {ticker}: volume_24h={volume_24h:.0f} < {settings.MIN_VOLUME_24H}")
                        report.filtered.append(FilteredMarket(
                            ticker=ticker, city=city_key, title=title,
