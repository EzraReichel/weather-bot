...
CLIMATOLOGY_DEVIATION_MAX  = 1.5   # °F — skip if ensemble mean is within this of climatology
...
async def generate_weather_signal(market: WeatherMarket) -> Optional[WeatherTradingSignal]:
    ...
    try:
        from weatherbot.data.multi_source_weather import fetch_all_sources
        ...
        if (
            ...
            if _gfs_members and _ecmwf_members:
                ...
                if _divergence > MODEL_DIVERGENCE_THRESHOLD:
                    logger.info(
                        f"SKIP {market.market_id}: GFS mean={_gfs_mean:.1f}F vs "
                        f"ECMWF mean={_ecmwf_mean:.1f}F — divergence {_divergence:.1f}F "
                        f"exceeds {MODEL_DIVERGENCE_THRESHOLD:.0f}F threshold (model_divergence)"
                    )
    ...
    if market.metric in ("high", "low"):
        try:
            from weatherbot.data.multi_source_weather import fetch_current_observation
            ...
            if 0 < _hours_left <= OBS_WINDOW_HOURS:
    ...
    if climo_normal is not None and market.metric in ("high", "low"):
        ...
        if deviation <= CLIMATOLOGY_DEVIATION_MAX:
            logger.info(
    ...
    cold_day_exception = (
        ...
        and cold_margin >= COLD_DAY_MARGIN
        and nws_prob >= COLD_DAY_NWS_MIN
    )
    yes_min_entry = settings.WEATHER_MIN_ENTRY_PRICE if cold_day_exception else YES_ENTRY_FLOOR
...
async def scan_for_weather_signals() -> ScanReport:
    ...
    from weatherbot.data.kalshi_markets import fetch_kalshi_weather_markets, MarketFetchReport

    if settings.CITY_OVERRIDE:
        city_keys = [settings.CITY_OVERRIDE.strip()]
    else:
        city_keys = [k for k, v in CITY_CONFIG.items() if v.get("enabled", True)]
