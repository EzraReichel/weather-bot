...
def log_paper_trade(signal) -> Optional[Trade]:
    ...
    Returns the Trade row, or None if it was already logged this session.
    """
    _ensure_db()
    ...
    try:
        ...
        same_city_trades = db.query(Trade).filter(
...
async def settle_paper_trades() -> List[Trade]:
    """
...
def get_paper_stats(db=None):
    ...
    try:
        all_trades  = db.query(Trade).filter(Trade.is_paper == True).all()
